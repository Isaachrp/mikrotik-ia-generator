...
class RouterosAPI
{
                ...
                if (isset($RESPONSE[0]) && $RESPONSE[0] == '!done') {
                    ...
                    if (preg_match_all('/[^=]+/i', $RESPONSE[1], $MATCHES)) {
                        if ($MATCHES[0][0] == 'ret' && strlen($MATCHES[0][1]) == 32) {
                            ...
                            { //Router OS Versiones 6.43 o superiror
                                $this->write('/login',false);
                                $this->write('=name=' . $login,false);
    ...
    public function comm($com, $arr = array())
    {
     ...
     public function write2($command, $param2 = true)
    {
        if ($command) {
            $data = nl2br($command);
            $data = str_ireplace("<br />", "", $data);
            fwrite($this->socket, $this->encodeLength(strlen($data)) . $data);
            $this->debug('<<< [' . strlen($data) . '] ' . $data);
    ...
    public function comm2($com, $arr = array())
    {
        $count = count($arr);
        $this->write2($com, !$arr);
        $i = 0;
